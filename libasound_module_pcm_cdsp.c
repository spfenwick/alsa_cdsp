...
typedef struct {
  ...
  // cargs[1+] = Additional arguments passed through .asoundrc
  // cargs will also be used to hold hw_params
...
static int start_camilla(cdsp_t *pcm) {
  ...
  switch (pcm->io.format) {
    case SND_PCM_FORMAT_S16_LE:
      snprintf(sformat, sizeof(sformat), "S16_LE");
      break;
    case SND_PCM_FORMAT_S24_LE:
      snprintf(sformat, sizeof(sformat), "S24_4_LJ_LE");
      break;
    case SND_PCM_FORMAT_S24_3LE:
      snprintf(sformat, sizeof(sformat), "S24_3_LE");
      break;
    case SND_PCM_FORMAT_S32_LE:
      snprintf(sformat, sizeof(sformat), "S32_LE");
      break;
    case SND_PCM_FORMAT_FLOAT_LE:
      snprintf(sformat, sizeof(sformat), "F32_LE");
      break;
    case SND_PCM_FORMAT_FLOAT64_LE:
      snprintf(sformat, sizeof(sformat), "F64_LE");
      break;
    default:
      // Shouldn't get here
...
SND_PCM_PLUGIN_DEFINE_FUNC(cdsp) {
  ...
  pcm->n_cargs = 1; // First one is proc name
